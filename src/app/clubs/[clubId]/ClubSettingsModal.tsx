...
interface ClubSettingsModalProps {
  ...
  onClose: () => void;
  club: any;
  setClub: (club: any) => void;
}

export default function ClubSettingsModal({ isOpen, onClose, club, setClub }: ClubSettingsModalProps) {
  ...
  const fetchBans = async () => {
    try {
      const res = await fetch(`/api/clubs/${club.id}/bans`);
      if (res.ok) {
  ...
  const handleGenerateJoinCode = async () => {
    try {
      const res = await fetch(`/api/clubs/${club.id}/manage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        const data = await res.json();
        setClub({ ...club, joinCode: data.joinCode });
  ...
  const handleKickMember = async (userId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setClub({
          ...club,
          members: club.members.filter((m: any) => m.userId !== userId)
        });
      }
    } catch (error) {
  ...
  const handleUnbanUser = async (userId: string) => {
    ...
    try {
      const res = await fetch(`/api/clubs/${club.id}/manage?userId=${userId}`, {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white/80 dark:bg-zinc-950/80 backdrop-blur-sm overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <div className="min-h-screen py-12 px-6 flex justify-center">
        <div className="w-full max-w-6xl bg-white dark:bg-zinc-950 rounded-2xl shadow-2xl border border-zinc-100 dark:border-zinc-900 p-8 md:p-12">
          ...
          {/* Unified Layout */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-zinc-900 dark:text-zinc-100">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="border border-zinc-100 dark:border-zinc-900 rounded-xl p-8 bg-white dark:bg-zinc-950 shadow-sm relative overflow-hidden">
                ...
                <form onSubmit={handleUpdateClub} className="space-y-8">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-baseline">
                      <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-400 ml-1">Rules / Terms & Conditions</label>
                      <span className="text-[9px] font-mono text-zinc-400">{rules.length}/1000</span>
                    </div>
                    <textarea 
                      value={rules}
                      ...
                      maxLength={1000}
                      rows={5}
                      placeholder="Members must agree to these terms before joining..."
                      className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded outline-none focus:border-zinc-900 dark:focus:border-white transition-all text-xs leading-relaxed resize-none font-medium"
                    />
                  </div>
